...
# -- Main config --
for config_name in ./configs/*.toml; do
	toml_prep "${1:-$config_name}" || abort "could not find config file '${1:-$config_name}'\n\tUsage: $0 <config.toml>"
	...
	for table_name in $(toml_get_table_names); do
		...
		app_args[module_prop_name]=$(toml_get "$t" module-prop-name) || app_args[module_prop_name]="${table_name_f}-jlambert360"
